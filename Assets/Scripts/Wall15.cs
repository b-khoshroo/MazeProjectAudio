...
public class Wall15 : MonoBehaviour
{
    AudioSource wall15_audioSource;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.JoystickButton2))
        {
            ...
            if ((GameObject.Find("Player").transform.position.x > transform.position.x)
                && (GameObject.Find("Player").transform.position.x < transform.position.x + 7))
            {
        ...
        if (Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.JoystickButton1))
        {
            ...
            if ((GameObject.Find("Player").transform.position.x < transform.position.x)
                && (GameObject.Find("Player").transform.position.x > transform.position.x - 10))
            {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            wall15_audioSource = GetComponent<AudioSource>();
            wall15_audioSource.Play();
